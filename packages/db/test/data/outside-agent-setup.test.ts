...
/**
 * The answer to installing Patcher's skills for agents outside Patcher (#141).
 *
 ...
 */
describe("the answer about agents outside Patcher", () => {
  let db: DbConnection;
  ...
  it("is unasked on an install with no settings row", () => {
    expect(getOutsideAgentSetup(db)).toBe("unasked");
  });
  ...
  it("reads text nobody can parse as unasked", () => {
    setOutsideAgentSetup(db, "accepted");
    db.$client
      .prepare("UPDATE app_settings SET outside_agent_setup = 'maybe'")
      .run();
