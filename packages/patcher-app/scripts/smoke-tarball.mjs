...
const tempRoot = await mkdtemp(join(tmpdir(), "patcher-app-tarball-"));
...
async function smokeBridgeModelList({
  ...
  childProcess.stdin.write(
    `${JSON.stringify({
      ...
      id: 1,
      method: "initialize",
      params: { clientInfo: { name: "patcher-app-smoke", version: "0.0.0" } },
    })}\n`,
  );
  childProcess.stdin.write(
    `${JSON.stringify({
...
async function smokePiUserConfiguration(packageDir) {
    ...
    onMessage(message) {
      ...
      childProcess.stdin.write(
        `${JSON.stringify({
          ...
          id: message.id,
          result: {
            contentItems: [{ type: "inputText", text: "Patcher tool result" }],
            success: true,
          },
        })}\n`,
      );
  ...
  try {
    sendBridgeRequest(childProcess, 101, "initialize", {
      clientInfo: { name: "patcher-app-smoke", version: "0.0.0" },
    });
      ...
      dynamicTools: [
        {
          ...
          description: "A tool provided by Patcher.",
          inputSchema: {
            type: "object",
    ...
    if (dynamicToolCalls.length !== 1) {
      throw new Error(
        `${label} expected one Patcher tool call, received ${dynamicToolCalls.length}`,
      );
    }
    ...
    if (
      ...
      dynamicToolCall.params.arguments.value !== "Patcher tool input"
    ) {
      throw new Error(
        `${label} received an invalid Patcher tool call: ${JSON.stringify(dynamicToolCall)}`,
      );
    }
...
async function smokeHelpCommands(tarballPath) {
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher-app", ["--help"]),
    command: "npx",
    label: "patcher-app help",
  });
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher", ["--help"]),
    command: "npx",
    label: "Patcher cli help",
  });
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher-server", ["--help"]),
    command: "npx",
    label: "patcher-server help",
  });
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher-host-daemon", ["--help"]),
    command: "npx",
    label: "patcher-host-daemon help",
  });
}
...
async function smokeConfigCommand(tarballPath) {
  ...
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher-app", [
      "--data-dir",
      dataDir,
      "env",
      "set",
    ...
    command: "npx",
    label: "patcher-app env OPENAI_API_KEY",
  });
  await runCommand({
    args: createNpxArgs(tarballPath, "patcher-app", [
      "--data-dir",
    ...
    command: "npx",
    label: "patcher-app config PATCHER_APP_URL",
  });
  ...
  if (envJson.env?.OPENAI_API_KEY !== "test-openai-key") {
    throw new Error("Expected patcher-app env to persist OPENAI_API_KEY");
  }
  if (configJson.config?.PATCHER_APP_URL !== "https://patcher.example.test") {
    throw new Error("Expected patcher-app config to persist PATCHER_APP_URL");
  }
}
...
async function smokeSdkPackage(tarballPath) {
  ...
  await runCommand({
    ...
    command: "npm",
    cwd: sdkDir,
    label: "install patcher-app SDK smoke package",
  });
  await runCommand({
    args: [
      "--input-type=module",
      "-e",
      'import { PatcherSdk } from "patcher-app"; if (typeof PatcherSdk !== "function") process.exit(1);',
    ],
    command: "node",
    cwd: sdkDir,
    label: "patcher-app SDK JavaScript import",
  });
  await writeFile(
    join(sdkDir, "sdk-smoke.ts"),
    [
      'import { PatcherSdk, PatcherHttpError } from "patcher-app";',
      "",
      'const patcher = new PatcherSdk({ baseUrl: "http://127.0.0.1:38986" });',
      "const error: typeof PatcherHttpError = PatcherHttpError;",
      "void patcher.status.get();",
      "void error;",
      "",
    ].join("\n"),
  );
  await runCommand({
    ...
    command: "npx",
    cwd: sdkDir,
    label: "patcher-app SDK TypeScript import",
  });
  return sdkDir;
}
...
async function smokeBuiltinPluginsRunning({ cliEnv, tarballPath }) {
  ...
  while (Date.now() <= deadline) {
    const stdout = await runCommand({
      args: createNpxArgs(tarballPath, "patcher", ["plugin", "list", "--json"]),
      command: "npx",
      env: cliEnv,
      label: "patcher plugin list",
    });
...
async function smokeFullStack(tarballPath, sdkDir) {
  ...
  try {
    ...
    await runCommand({
      args: createNpxArgs(tarballPath, "patcher", ["status"]),
      command: "npx",
      env: cliEnv,
    ...
    await runCommand({
      args: [
        ...
        [
          ...
          "const patcher = new PatcherSdk({ baseUrl: process.env.PATCHER_SERVER_URL });",
          "await patcher.status.get();",
        ].join("\n"),
      ],
      command: "node",
      cwd: sdkDir,
...
async function smokeDaemonJoin(tarballPath) {
  ...
  const server = spawnManagedProcess({
    args: createNpxArgs(tarballPath, "patcher-server", [
      "--data-dir",
      serverDataDir,
      "--server-port",
      String(serverPort),
  ...
  try {
    ...
    daemon = spawnManagedProcess({
      ...
      label: "patcher-app host-daemon join",
    });
    await waitForHttp({
      label: daemon.label,
      processRef: daemon,
...
try {
  ...
  const installedPackageDir = join(sdkDir, "node_modules", "patcher-app");
  ...
  process.stdout.write("patcher-app tarball smoke passed\n");
} finally {
