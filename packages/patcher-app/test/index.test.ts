...
const startupOnlyManagedEnvCases: StartupOnlyManagedEnvCase[] = [
  ...
  { key: "PATCHER_DATA_DIR", value: "/tmp/patcher-managed-data" },
  ...
  { key: "PATCHER_INHERITED_SKILLS_ROOTS", value: "/tmp/patcher-skills" },
...
function createTestStartContext(): PatcherAppStartContext {
  return {
    appDistDir: "/tmp/patcher-app-test/app/dist",
    appVersion: "0.0.0-test",
    configFile: "/tmp/patcher-app-test/config.json",
    daemonBundleDir: "/tmp/patcher-app-test/host-daemon/dist",
    daemonEntry: "/tmp/patcher-app-test/host-daemon/dist/daemon-bundle.mjs",
    daemonLockDir: "/tmp/patcher-app-test/daemon.lock.lock",
    daemonLockFile: "/tmp/patcher-app-test/daemon.lock",
    daemonPort: 38987,
    dataDir: "/tmp/patcher-app-test",
    dbPath: "/tmp/patcher-app-test/patcher.db",
    envFile: "/tmp/patcher-app-test/env.json",
    logDir: "/tmp/patcher-app-test/logs",
    packageRoot: "/tmp/patcher-app-test/package",
    serverEntry: "/tmp/patcher-app-test/server/dist/index.js",
    serverPort: 38986,
...
describe("patcher-app launcher", () => {
  ...
  it("resolves production defaults for npx startup", () => {
    const context = resolvePatcherAppStartContext({
      entrypointUrl: pathToFileURL(
        "/repo/packages/patcher-app/dist/patcher-app.js",
      ).href,
      env: {},
      homeDir: "/home/tester",
    });
    ...
    expect(context.serverEntry).toBe(
      "/repo/packages/patcher-app/server/dist/index.js",
    );
    expect(context.daemonEntry).toBe(
      "/repo/packages/patcher-app/host-daemon/dist/daemon-bundle.mjs",
    );
  ...
  it("uses workspace build outputs when run from a source checkout", () => {
    const context = resolvePatcherAppStartContext({
      entrypointUrl: pathToFileURL("/repo/packages/patcher-app/src/launcher.ts")
        .href,
      env: {},
      homeDir: "/home/tester",
    });

    expect(context.packageRoot).toBe("/repo/packages/patcher-app");
  ...
  it("falls back to the dev sentinel when package.json is missing", () => {
    expect(readPatcherAppPackageVersion("/nonexistent/patcher-app/path")).toBe(
      "0.0.0-dev",
    );
  });
  ...
  it("passes the server bind host flag to the server environment", async () => {
    ...
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-bind-host-"));
  ...
  it("strips parent thread context from the production server without stripping the CLI", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-thread-context-"));
    const runtime = await resolvePatcherAppRuntimeState({
      entrypointUrl: pathToFileURL(
        "/repo/packages/patcher-app/dist/patcher-app.js",
      ).href,
  ...
  it("rejects an invalid server bind host before launcher startup", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-invalid-bind-host-"),
    );
  ...
  it("uses a supplied join code without requesting a loopback enroll key", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-remote-join-"));
  ...
  it("uses managed config server URL when env and flags omit it", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-config-"));
    writeFileSync(
  ...
  it("uses managed config server URL over ambient env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-server-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
  ...
  it("keeps full-stack startup local even when managed config has a server URL", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-local-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ serverUrl: "https://patcher.example.test" }),
      "utf8",
  ...
  it("applies managed config environment values over ambient env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-env-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({
        config: {
  ...
  it("uses launcher flags over managed config and ambient server URL", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-flag-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ serverUrl: "https://stored.example.test" }),
      "utf8",
  ...
  it("preserves customModels across managed config writes", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-config-custom-"));
    const customModels = [
      {
  ...
  it("preserves invalid customModels across managed config set writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-invalid-custom-models-set-"),
    );
    const customModels = [
  ...
  it("preserves customAcpAgents across managed config writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-custom-acp-"),
    );
  ...
  it("preserves invalid customAcpAgents across managed config set writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-invalid-custom-acp-set-"),
    );
    const customAcpAgents = [
      {
        id: "bad agent",
  ...
  it("preserves invalid customAcpAgents across managed config unset writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-invalid-custom-acp-unset-"),
    );
    const customAcpAgents = [
      {
        id: "bad agent",
  ...
  it("keeps secrets out of the config command", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-secret-config-"));
    ...
    ).rejects.toThrow(/patcher-app env set OPENAI_API_KEY/u);
  });
  ...
  it("rejects invalid server bind hosts before writing managed env", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-invalid-bind-env-"),
    );
  ...
  it("unsets an invalid server bind host already in managed env", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-recover-bind-env-"),
    );
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
  ...
  it("names the env file when a persisted server bind host is invalid", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-bad-bind-env-"));
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
      JSON.stringify({ env: { PATCHER_SERVER_BIND_HOST: "localhost" } }),
  ...
  it("blames the flag, not the env file, for an invalid flag value", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-bad-bind-flag-"));
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
      JSON.stringify({ env: { PATCHER_SERVER_BIND_HOST: "localhost" } }),
  ...
  it("rejects invalid env key names", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-invalid-env-"));
  ...
  it("uses the explicit server bind host flag over managed env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-bind-host-env-"));
    writeFileSync(
      join(dataDir, "env.json"),
  ...
  it("unsets managed env values", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-env-unset-"));
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({
        env: {
  ...
  it("rejects invalid managed config values before writing or reloading", async () => {
    ...
    try {
      for (const testCase of invalidConfigCommandCases) {
        const dataDir = mkdtempSync(
          join(tmpdir(), "patcher-app-invalid-config-"),
        );
  ...
  it("prints a restart notice when setting a startup-only config key", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-startup-config-set-"),
    );
    const server = await startConfigReloadTestServer();
    ...
    try {
      ...
      expect(output).toContain(
        "PATCHER_LOG_LEVEL is startup-only. The running process keeps its current value; a full patcher-app restart is required to apply this change. Run `patcher-app stop && patcher-app start`, or restart the desktop app.",
      );
  ...
  it("prints restart notices for every server startup-only managed env key", async () => {
    ...
    try {
      for (const testCase of startupOnlyManagedEnvCases) {
        const dataDir = mkdtempSync(
          join(tmpdir(), "patcher-app-startup-env-set-"),
        );
        const output = await captureStdout(() =>
          runPatcherApp([
            "--data-dir",
        ...
        expect(output).toContain(
          `${testCase.key} is startup-only. The running process keeps its current value; a full patcher-app restart is required to apply this change.`,
        );
        expect(output).not.toContain("Reloaded running Patcher server config.");
      }
  ...
  it("warns that wildcard exposure remains after unsetting the server bind host", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-startup-bind-unset-"),
    );
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({ env: { PATCHER_SERVER_BIND_HOST: "0.0.0.0" } }),
    ...
    try {
      ...
      expect(output).toContain(
        "PATCHER_SERVER_BIND_HOST is startup-only. The running process keeps its current value; a full patcher-app restart is required to apply this change. Run `patcher-app stop && patcher-app start`, or restart the desktop app.",
      );
      expect(output).toContain(
  ...
  it("keeps the reload confirmation for reloadable keys", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-reloadable-env-set-"),
    );
    const server = await startConfigReloadTestServer();
  ...
  it("reports that startup-only config will apply on next start without a running server", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-startup-next-start-"),
    );
  ...
  it("notes configured startup-only keys after explicit refresh", async () => {
    ...
    try {
      ...
      expect(output).toContain("Reloaded running Patcher server config.");
      expect(output).toContain(
        "Startup-only settings currently configured (PATCHER_FF_PLACEHOLDER, PATCHER_LOG_LEVEL, PATCHER_SERVER_BIND_HOST, PATCHER_SERVER_PORT, PATCHER_TELEMETRY) apply on the next full patcher-app restart.",
      );
    } finally {
      await server.close();
    }
  ...
  it("asks a running local server to reload after config writes", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-config-reload-"));
    const server = await startConfigReloadTestServer();
  ...
  it("supports explicitly refreshing running server config", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-config-refresh-"));
    const server = await startConfigReloadTestServer();
  ...
  it("uses PATCHER_SERVER_URL for config refresh when set", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-env-refresh-"),
    );
    const server = await startConfigReloadTestServer();
  ...
  it("uses persisted PATCHER_SERVER_URL for config refresh without env or flags", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "patcher-app-config-persisted-url-"),
    );
    const server = await startConfigReloadTestServer();
  ...
  it("uses --server-url over env and persisted config for config refresh", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "patcher-app-config-flag-url-"));
  ...
  it("detects npm bin symlinks as the main module", () => {
    const testDir = mkdtempSync(join(tmpdir(), "patcher-app-main-"));
    const realEntryPath = join(testDir, "dist-index.js");
    const symlinkPath = join(testDir, "patcher");
