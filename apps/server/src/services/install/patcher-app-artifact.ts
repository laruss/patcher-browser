...
async function readPatcherAppPackageJson(
  ...
  if (
    ...
    !("name" in parsed) ||
    parsed.name !== "patcher-app" ||
    !("version" in parsed) ||
    typeof parsed.version !== "string"
  ) {
    throw new Error(`Expected a patcher-app package at ${packageRoot}`);
  }
...
/**
 * Locates the patcher-app package by probing the layouts the server actually runs
 ...
 * build patcher-app before packing, a packaged install just packs itself.
 */
export async function resolvePatcherAppPackage(
  ...
  const candidates: readonly { layout: "packaged" | "repo"; root: string }[] = [
    ...
    {
      layout: "repo",
      root: resolve(serverEntryDir, "../../../packages/patcher-app"),
    },
  ];
  ...
  throw new Error(
    `Unable to locate the patcher-app package from ${serverEntryDir}; tried ${candidates
...
export function createPatcherAppArtifactService(
  ...
  async function buildTarball(): Promise<string> {
    ...
    const tarballPath = join(
      cacheDir,
      `patcher-app-${safeVersionFilePart(packageJson.version)}-protocol-${protocolVersion}.tgz`,
    );
    ...
    if (resolved.layout === "repo") {
      ...
      await commandRunner(
        "bunx",
        ["turbo", "run", "build", "--filter=patcher-app"],
        repoRoot,
      );
    }
    ...
    // `patcher-app-<version>.tgz`, which can never collide with the
