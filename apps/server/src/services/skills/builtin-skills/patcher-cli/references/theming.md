# patcher theme authoring reference
...
The folder name is the theme id. Run `patcher theme dir` to print the exact directory
...
and re-run `patcher theme set <name>` (or just re-select it) to re-apply.
...
1. `patcher theme dir` — print the custom-theme directory (e.g. `~/.patcher/theme`).
...
3. `patcher theme set <name>` — activate it. To edit later, change the file in place
   and re-run `patcher theme set <name>`.

Other commands:

- `patcher theme set <id> [--favicon-color <color>]` — switch to a built-in
...
- `patcher theme show` — print the active palette; `--css` dumps the active theme's
  CSS. `patcher theme list` shows the active palette and all discovered themes;
  `patcher theme reset` returns to `default` without changing the favicon color.
- `patcher theme favicon set <color>` / `patcher theme favicon reset` — update or reset
