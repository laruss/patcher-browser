...
describe("tab ownership", () => {
  ...
  it("prefers a turn's own tab over the person's once it has one", async () => {
    const harness = createHarness({
      state: {
        activeTabId: "a",
        tabs: [tab("a", "https://person.example/"), tab("b", "https://mine.example/")],
      },
      issuer: TURN,
  ...
  it("refuses another agent's tab to everyone, and asks nobody for it", async () => {
    const harness = createHarness({
      state: { activeTabId: "a", tabs: [tab("a"), tab("b", "https://theirs.example/")] },
      issuer: TURN,
    ...
    expect(outcome.code).toBe("tab_not_yours");
    // Named, because "not yours" without a name is a dead end.
